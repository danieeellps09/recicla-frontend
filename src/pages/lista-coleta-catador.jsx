...
const AdicionarColeta = (props) => {
    ...
    const handleVeiculosChange = (event) => {
        const veiculo = veiculos.find(v => v.nomeVeiculo === event.target.innerText);
        ...
        axios.post('https://18.189.110.174/api/v1/forms/coleta', dataToSend, config)
            .then(response => {
                ...
                if (response && response.data) {
                    ...
                    toast.error('Erro ao preencher coleta. Resposta inválida do servidor.',);
                }
            })
    ...
    return (
        ...
        >
            ...
            <Modal.Body>
                ...
                <Row className="w-100 my-3">
                    <Form>
                        <Form.Label className="text-orange" placeholder="dia/Mes/Ano">Data da Coleta: </Form.Label>
                        {/* Utilizando o componente InputMask para adicionar uma máscara */}
                        <InputMask mask="99/99/9999" maskChar="_" value={data}
                ...
                <Row className="w-100 my-3">
                    <Col>
                        ...
                        <Dropdown className="w-100">
                            ...
                            <Dropdown.Menu className="w-100 ">
                                <Dropdown.Item onClick={() => handlePerguntaChange('Sim')}>Sim</Dropdown.Item>
                                <Dropdown.Item onClick={() => handlePerguntaChange('Não')}>Não</Dropdown.Item>
                            </Dropdown.Menu>
                        </Dropdown>
                    </Col>
...
const Visualizar = (props) => {
    ...
    const [coletaSelecionadoId, setColetaSelecionadoId] = useState(props.idColeta); // Utilizando o ID do catador passado como prop
...
function ListarColetasCatador() {
    ...
    const filteredColetas = coletaData
        ? coletaData.filter(coleta => {
            ...
            const quantidadeMatches = coleta.quantidade.toString().includes(searchString);
    ...
    useEffect(() => {
        axios
            .get(
                "https://18.189.110.174/api/v1/forms/coleta/coletas/coletas-by-catadores",
                config
            )
            .then((response) => {
            ...
            .catch((error) => {
                ...
                toast.error('Erro ao lista coletas. Verifique os dados e tente novamente.');
        ...
        >
            <Row className="border bg-white rounded-5 shadow mt-5 w-100 justify-content-center p-5">
                ...
                >
                    ...
                    <AdicionarColeta show={modalAdicionarShow} onHide={() => setModalAdicionarShow(false)} />
                </Col>
                <Col>
                    ...
                    <ListGroup as="ol" numbered>
                        {paginateResults(filteredColetas, currentPage, resultsPerPage).map((coleta, index) => (
                            ...
                            >
                                <div className="ms-2 me-auto">
                                    ...
                                    Data da Coleta: {format(new Date(coleta.dataColeta), 'dd/MM/yyyy')} <br />
